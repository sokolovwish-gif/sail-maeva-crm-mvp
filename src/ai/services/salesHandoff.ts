...
const salesPatterns: Array<{ intent: AIIntent; reason: string; pattern: RegExp }> = [
  ...
  { intent: "booking_payment", reason: "вопрос про бронь", pattern: /брон|забронировать|место за мной|закрепить|оставь место|я в деле|беру/i },
  { intent: "contract", reason: "вопрос про договор или чек", pattern: /договор|чек|акт|оферт|юрид|расписк/i },
  { intent: "date_unavailable", reason: "вопрос про точное наличие мест", pattern: /есть места|наличие|свободн|точно есть|сколько мест|мест осталось/i },
  { intent: "price", reason: "вопрос про точную цену", pattern: /точн.*цен|точн.*стоим|сколько стоит|цена|стоимость|прайс|скидк|рассроч|дешевле/i },
  ...
  { intent: "unknown", reason: "точные детали яхты или размещения", pattern: /какая яхта|фото яхты|отдельная каюта/i }
];
